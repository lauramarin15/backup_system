 ...
 * La L al final hace que sean tipo long (64 bits).
 * Sin la L, 1024*1024*1024 desborda un int de 32 bits. */
#define SIZE_1KB   (1024L)
#define SIZE_1MB   (1024L * 1024L)
#define SIZE_1GB   (1024L * 1024L * 1024L)

/* Rutas temporales — guardadas en output/ del proyecto */
#define TMP_SRC    "output/bk_src.bin"
#define TMP_SMART  "output/bk_dst_smart.bin"
#define TMP_STDIO  "output/bk_dst_stdio.bin"
...
run_benchmark(void)
{
    /* mkdir crea output/ si no existe.
     * EEXIST significa que ya existe — eso es normal, no es error. */
    if (mkdir("output", 0755) != 0 && errno != EEXIST) {
        fprintf(stderr, "[ERROR] No se pudo crear output/: %s\n",
                strerror(errno));
        return;
    }
...
run_backup(const char *src, const char *dst)
{
    ...
    snprintf(dst_smart, sizeof(dst_smart), "output/%s.smart", dst);
    snprintf(dst_stdio,  sizeof(dst_stdio),  "output/%s.stdio",  dst);
